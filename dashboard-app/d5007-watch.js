// Watch D-5007 until the mqtt-lib 2.3.0 build is deployed, then verify the full loop.
...
c.on('connect', () => {
  log('connected, watching ' + dev + ' for mqtt-lib 2.3.0');
...
c.on('message', (t, p) => {
  const s = p.toString();
  if (phase === 'watch' && s.includes('mqtt-lib 2.3.0')) {
    phase = 'verify';
